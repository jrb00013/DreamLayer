...
def build_image(dockerfile_path: str, tag: str, build_args: dict = None) -> bool:
    """Build a Docker image from a Dockerfile with real-time log output."""
    try:
        print(f"Building image {tag} from {dockerfile_path}")
        build_path = os.path.dirname(os.path.abspath(dockerfile_path))
        ...
        print(f"Error building image: {str(e)}", flush=True)
        return False
